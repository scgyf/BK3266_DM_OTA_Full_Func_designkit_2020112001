...
#ifdef BEKEN_OTA_BLE
const GATT_DB_t GATT_DB_SELECT[DB_LEN]={USER_DEF_OAD_GATT_DB};
#else
 ...
 *****************************************************************************************************/
u_int8 _GATTserver_Add_16ByteUUID_Characteristic_Value(u_int16 handle,u_int8* char_UUID,u_int8 local_notify,u_int8 permissions,u_int16 val_len,u_int8* p_value)
{
#if (PRH_BS_CFG_SYS_LE_SERVER_SUPPORTS_128_BIT_UUIDS==1)
    t_attrib* p_current_attrib = &GATTserver_DB[GATTserver_DB_next_index];
...
u_int8 GATTserver_Read_By_Group_Type(u_int16 max_result_len,u_int16 start_handle,u_int16 end_handle,u_int16 attrib_UUID,
                                     u_int8* p_result, u_int8* p_result_len, u_int8* pair_len)
{
    u_int16 i=0;
    ...
    *pair_len = 0x00;
    ...
    while ((i<=GATTserver_DB_last_index) && (GATTserver_DB[i].attrib_handle <= end_handle) && (((*p_result_len)+6) < max_result_len) )
    {
        ...
        if ((p_current_attrib->attrib_UUID.UUID_val == attrib_UUID) && (p_current_attrib->permissions & GATT_PERMISSION_READ))
        {
            ...
            if((*pair_len) && (*pair_len != p_current_attrib->attrib_value.service.service_UUID_len))
            {
                break;
            }

            *pair_len = p_current_attrib->attrib_value.service.service_UUID_len;
            ...
            if (p_current_attrib->attrib_value.service.service_UUID_len == 0x02)  //16_bit uuid
            {
            ...
            if(p_current_attrib->attrib_value.service.service_UUID_len == 0x10)   //128_bit uuid
            {
...
u_int8 GATTserver_Read_Long_Characteristic_Value_Offset(u_int16 max_result_len,u_int16 att_handle, u_int16 offset, u_int8* p_result,u_int8* p_result_len)
{
    ...
    // Now get the pointer to the Attrib value and check attrib length.
    {
        extern t_ATT_config ATT_config;
        u_int8* p_val = p_current_attrib->attrib_value.value.p_value;
        u_int16 length = p_current_attrib->attrib_value.value.max_len;
...
#endif
u_int8 GATTserver_Write_Command(u_int16 attrib_handle, u_int8* p_attrib_value,u_int16 length)
{
    ...
    else
    {
        ...
        // Now write the Value...
        {
            ...
            u_int16 max_len = p_current_attrib->attrib_value.value.max_len;
...
#ifdef BEKEN_OTA_BLE
                    app_ota_ble_pkt_decode(GATTserver_DB[i].attrib_handle, p_value, length);
#endif
...
t_GATT_error GATTserver_Characteristic_Write_Local_Value(u_int16 char_val_handle,u_int16 length, u_int8* p_attrib_value)
{
    u_int8 i=0;
    u_int16 j=0;
    u_int16 char_UUID;
    ...
    if((BTq_Get_Queue_Number_Of_Entries(L2CAP_OUT_Q, p_connection->device_index) + \
        BTq_Get_Queue_Number_Of_Entries(LMP_OUT_Q, p_connection->device_index)) >= (PRH_BS_CFG_SYS_NUM_OUT_ACL_PACKETS - 2))
    {
        os_printf("!!!LE queue data full!!!\r\n");
    ...
    else
    {
        ...
        {
            //u_int8 value_len = p_current_attrib->attrib_value.value.val_len;
            u_int16 value_len = p_current_attrib->attrib_value.value.max_len;
            u_int8* p_value = p_current_attrib->attrib_value.value.p_value;
