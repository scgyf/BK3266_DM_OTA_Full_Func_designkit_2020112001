...
extern u_int16 ancs_handle_cache[HDL_CACHE_LEN];
#if(MOREDATA_DISABLE)
extern u_int8 moredata_enable;
#endif

/*
 * Callback functions which are registered with the lower layers.
 */
void Init_GAP_Discovery(t_LE_Connection* p_connection);
#if(PRH_BS_CFG_SYS_LE_GATT_INCLUDED==1)
...
#else
//0x2A29   长度  类型  数据
u_int8  EQ_CONFIG_adv_data[]= {
    /* Flags Field */ 0x02,0x01,0x06 /*GAP_LE_BR_EDR_NOT_SUPPORTED*/,
    /* Device name */ 0x12,0x09,'B','K','3','2','6','6','D','M','_','B','L','E','_','0','0','0','0', //0x09:Complete Local Name
...
#endif
u_int8  EQ_CONFIG_scan_resp_data[] = 
    {
	/* Device name */ 0x12,0x08,'B','K','3','2','6','6','D','M','_','B','L','E','_','0','0','0','0',  //0x08:Shortened Local Name,suggest same to adv local name when scaned by Andriod v7.0 above
    };
#if 0
...
#endif
void App_Set_BLE_Addr_Name(void)
{
    uint8 i,j;
    uint8 bt_addr_name[38];
    app_env_t env_h;

    memcpy((uint8 *)&env_h.env_cfg, (uint8 *)FLASH_ENVCFG_DEF_ADDR, sizeof(app_env_cfg_t));
    flash_read_data((uint8*)&bt_addr_name, (uint32)FLASH_CHANGED_BT_ADDR, 38);

    j = 0;
    for(i = 0; i < 6; i ++)
    {
        if(bt_addr_name[i]!=0xff)
            j++;
    }
    //suppose the number of !0xff in BTaddress is greater than 1
    if(j > 1)
    {
        memcpy((uint8*)&env_h.env_cfg.bt_para.device_addr,&bt_addr_name[0],6);
    }
    ...
    if(j > 2) //suppose the valid character number in BTname is greater than 2
    {
        ...
        memcpy((uint8*)&env_h.env_cfg.bt_para.device_name[0],&bt_addr_name[6],j);
    }

    for(i = 0; i < 6; i++)
    {
        LEconfig_device_address[i] = env_h.env_cfg.bt_para.device_addr.b[i];
        if(i < 2)
        {
            EQ_CONFIG_adv_data[20-i*2] = EQ_CONFIG_scan_resp_data[17-i*2] = ((LEconfig_device_address[i]>>4)&0x0F)+((((LEconfig_device_address[i]>>4)&0x0F)<0x0A)?48:55);
            EQ_CONFIG_adv_data[21-i*2] = EQ_CONFIG_scan_resp_data[18-i*2] = (LEconfig_device_address[i]&0x0F)+(((LEconfig_device_address[i]&0x0F)<0x0A)?48:55);
        }
        else
        {
            LEconfig_device_address[i] ^= (i+10);
        }
    }
    ...
    for(i = 0; i < 6; i++)
    {
        os_printf("%x", LEconfig_device_address[5-i]);
        if(i < 5)
            os_printf(":");
    }
    os_printf("\r\n");
    ...
    for(i = 5; i < 22; i++)
    {
        os_printf("%c", EQ_CONFIG_adv_data[i]);
    }
    os_printf("\r\n");
    os_printf("--------------------------\r\n");
}
...
void LE_APP_Init(void)
{
   ...
   u_int8  peer_address[6] = {0x60, 0x32, 0xC3, 0x60, 0x32, 0xC3};
   #endif
   #if(APP_ANCS)
    LEapp_Config.gap_discovery_request_timer = BT_TIMER_OFF;
    LEapp_Config.SMP_security_request_timer= BT_TIMER_OFF;
    LEapp_Config.ancs_config_notify_timer=BT_TIMER_OFF;
    LEapp_Config.ancs_config_data_timer=BT_TIMER_OFF;
    #endif
    ...
    LEsmp_Configure_Security(SMP_NoInputNoOutput /* io_capability - SMP_NoInputNoOutput  */, 0x00 /* oob_flag - Data Not Present */,SMP_AUTHREQ_BONDING_NO_MITM /* auth_req - Bonding / !MITM */,
    ...
    //GAP_Set_AD_Data(sizeof(HID_adv_data),(u_int8*)HID_adv_data);
	
    GAP_Set_ScanResp_Data(sizeof(EQ_CONFIG_scan_resp_data),(u_int8*)EQ_CONFIG_scan_resp_data);//C_Chip
    //GAP_Set_ScanResp_Data(sizeof(HID_scan_resp_data),(u_int8*)HID_scan_resp_data);
    ...
    GAP_Connection_Establishment(GAP_GENERAL_CONNECT,LEconfig_device_address,PUBLIC_ADDRESS,24,24,0,72);
#endif
#endif
}
...
//unsigned char temp_gpio = 0;
	...
	//u_int8 s[8],d[8];
    //os_printf("LE_APP_GATT_Event_Handler eventCode:0x%x\r\n",eventCode);	
    switch(eventCode)
    {
    ...
    case GATT_VALUE_NOTIFICATION_EVENT :
        ...
        {
            #if(APP_ANCS)
            get_ancs_notification(p_gatt_event_params->payload,p_gatt_event_params->payload_length);
            #endif
        }
        else
        {
    ...
    {
        #if(APP_ANCS)
        os_printf("CHARACTERISTICS_EVENT\n");
    ...
    case  GATT_WRITE_COMPLETE_EVENT:
    {
              ...
              switch(p_gatt_event_params->handle)
           	 {
      		    case EQ_PARAM0_HANDLE:
           		    {
				        //temp_gpio = p_gatt_event_params->payload[0];
                        //os_printf("GPIO en: 0x%x\r\n",temp_gpio);
           		    	//EQ_Data_Slave_converter(p_gatt_event_params->payload,aud_equ_ble);
           		    	//w_eq_count++;
                        //LMutils_Get_Uint16(p_gatt_event_params->payload);
           		    }  
			    break;

           	    case EQ_PARAM1_HANDLE:
           		    {
                        //temp_gpio = p_gatt_event_params->payload[0]-0x30;
                        //os_printf("GPIO dis: 0x%x\r\n",temp_gpio);
           		      	//EQ_Data_Slave_converter(p_gatt_event_params->payload,aud_equ_ble+1);
           		    	//w_eq_count++;
           		    }  
			    break;
        	...
        	//   os_printf("handle = %x ,length %d, value = %d\r\n",p_gatt_event_params->handle, p_gatt_event_params->payload_length,p_gatt_event_params->payload[0]);
           }
	break;
...
void LE_App_Local_Disconnect_Done(void)
{
...
///check bt disconnect ///add 210219
...
u_int8 LE_App_Local_Disconnect_Indication(t_error reason)
{
  ...
  u_int8  peer_address[6] = {0x61, 0x33, 0xcc, 0xdd, 0xee, 0xff};
...
#endif
    ble_connected_flag = 0;
    Gatt_Set_MTU_Size(23);   //Must be set default value 23, do not change!!!
   ...
   GAP_Connection_Establishment(GAP_GENERAL_CONNECT,peer_address,PUBLIC_ADDRESS,24,24,0,72);
    ...
    ancs_state_set(0);
#endif
    os_printf("LE disconnected: 0x%x\r\n", reason);
    return NO_ERROR;
}
...
void LE_APP_GAP_Connection_Complete(u_int8 event,t_gap_event_params* p_event_params )
{
...
#ifdef GAP_MASTER_MODE
    uint8_t  peer_address[6] = {0x61, 0x33, 0xcc, 0xdd, 0xee, 0xff};
#endif
    ...
    if (p_event_params->status == NO_ERROR)
    {
        ...
        LE_App_Set_L2cap_Para_Update_Timer(current_clk + 3200*2);  // 3200 = 1s, 2s
        ...
        if( (current_clk+GAP_START_DISCOVERY_TIME)>BT_CLOCK_MAX_TICKS )
        {
            LEapp_Config.gap_discovery_request_timer=(current_clk+GAP_START_DISCOVERY_TIME)-BT_CLOCK_MAX_TICKS;
        }
        else
        {
            LEapp_Config.gap_discovery_request_timer = current_clk+GAP_START_DISCOVERY_TIME;
        }
        #endif
...
void User_App_Check_Timers(void)
{
    ...
    {
        if(LEconnection_LE_Connected() && ancs_state_get())
        {
            LEapp_Config.SMP_security_request_timer = BT_TIMER_OFF;
    ...
    if  ((LEapp_Config.gap_discovery_request_timer!=  BT_TIMER_OFF) &&
            (((LEapp_Config.gap_discovery_request_timer - current_clk - 1) & BT_CLOCK_MAX_TICKS) >= BT_CLOCK_MAX_TIMEOUT))
    {
        if(ble_connected_flag)
        {
            Notify_BLE_Connected();
            LEapp_Config.gap_discovery_request_timer = BT_TIMER_OFF;
            Init_GAP_Discovery(p_connection);
        }
    }
    
    if  ((LEapp_Config.ancs_config_notify_timer!=  BT_TIMER_OFF) &&
            (((LEapp_Config.ancs_config_notify_timer - current_clk - 1) & BT_CLOCK_MAX_TICKS) >= BT_CLOCK_MAX_TIMEOUT))
    {
        if(GATT_Check_Is_Link_Encrypted())
        {
            LEapp_Config.ancs_config_notify_timer = BT_TIMER_OFF;
            Ancs_subsNotifSrc(ancs_handle_cache[HDL_ANCS_NTF_CCCD]);
            LEapp_Config.ancs_config_data_timer=current_clk+500;
        }
    }
    if  ((LEapp_Config.ancs_config_data_timer!=  BT_TIMER_OFF) &&
            (((LEapp_Config.ancs_config_data_timer - current_clk - 1) & BT_CLOCK_MAX_TICKS) >= BT_CLOCK_MAX_TIMEOUT))
    {
        if(GATT_Check_Is_Link_Encrypted())
        {
            LEapp_Config.ancs_config_data_timer = BT_TIMER_OFF;
            Ancs_subsNotifSrc(ancs_handle_cache[HDL_ANCS_DATA_SRC_CCCD]);
        }
    }
        ...
        (((LEapp_Config.TAPP_L2CAP_PARA_UPDATE_Timer - current_clk - 1) & BT_CLOCK_MAX_TICKS) >= BT_CLOCK_MAX_TIMEOUT))
    {
        ...
        if(1)
	    {
                ...
                //LE_L2CAP_Connection_Update(0xFFFF,24,(24-8),20,/*0x0258*/0x012c); ///interval 30ms,latency 20,timeout 3s
                LE_L2CAP_Connection_Update(0xFFFF,24,(24-8),20,0x258); ///interval 30ms,latency 20,timeout 3s
...
void LE_L2CAP_Response_Event_Handler(u_int8 eventCode, u_int8 status)
{
    ...
    if(eventCode==L2CAP_CONNECTION_PARAMETER_UPDATE_RESPONSE)
    {
        ...
        os_printf("L2CAP_CONNECTION_PARAMETER_UPDATE_RESPONSE!\n");
    }
    else if(eventCode==L2CAP_CONNECTION_PARAMETER_UPDATE_REQUEST)
    {
        ...
        os_printf("Update ok!connInterval_new = 0x%x,latency_new = 0x%x,timeout_new = 0x%x\n",
        		p_connection->connInterval_new,p_connection->latency_new,p_connection->timeout_new);
    }
}
...
#if 0
		// DAC HPF2 禁能位，1：绕开HPF2  写1 绕开HPF2
