...
void app_bt_enter_sniff_mode(uint16_t link_handle,uint8_t enable)
{
    ...
    if(enable)
    {
        ...
        sniff_cp.max_interval = 0x3E0; /* 0x3E0=620ms */
        sniff_cp.min_interval = 0x3C0; /* 0x200=320ms;0x280=400ms;0x2A0=420ms;0x2D0=450ms */
#else
...
void app_led_clear_pairing(void)
{
    app_handle_t app_h = app_get_sys_handler();
...
uint8 app_ble_latency_pass_enable(void)
{
    return 1;
}
#endif
...
#define con_power_level_max 0x0A
#define con_power_step 5
uint8 app_get_power_level(uint8 index)
{
	uint8 level=0x0A;
    app_env_handle_t  env_h = app_env_get_handle();
    ...
    if( env_h->env_cfg.used == 0x01 )
	{
		level = (env_h->env_cfg.feature.a2dp_rf_pwr.big_adj)&0x1f;
	}
	switch(index)
	{
		case 0:
			#if(con_power_level_max>(2*con_power_step))
			level -= con_power_step*2;
			#else
			level = 0;
			#endif
			break;
		case 1:
			#if(con_power_level_max>(con_power_step))
			level -= con_power_step;
			#else
			level = 0;
			#endif
			break;
