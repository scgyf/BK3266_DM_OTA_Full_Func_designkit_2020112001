...
#ifdef BEKEN_OTA
    env_cali_ptr = (uint8 *)FLASH_ENVDATA2_DEF_ADDR_16M;	
#else
...
int app_env_keypair_used_delete( btaddr_t *addr )
{
    ...
    for( i = 0; i < MAX_KEY_STORE; i++ )
    {
        ...
        && (btaddr_same( addr, &(env_h->env_data.key_pair[i].btaddr))) )
        {
            ...
            env_h->env_data.key_pair[i].used = 0xff;
            memset(&(env_h->env_data.key_pair[i].btaddr),0xff,6);
            memset(env_h->env_data.key_pair[i].linkkey,0xff,HCI_KEY_SIZE);
        #else
...
#if defined(CONFIG_ACTIVE_SSP)||defined(SUPPORT_BAIDU_APK)
    MSG_T msg;
    msg.id = MSG_ENV_WRITE_ACTION;
    msg.arg = 0x01;
    msg.hdl = 0;
