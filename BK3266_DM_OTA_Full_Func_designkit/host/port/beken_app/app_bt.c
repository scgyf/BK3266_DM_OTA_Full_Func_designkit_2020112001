...
#endif
#ifdef SUPPORT_BAIDU_APK
static uint8_t s_is_incoming_acl_connect = 0;
uint8_t bt_get_incoming_acl_connect(void)
{
    return s_is_incoming_acl_connect;     
}
void bt_set_incoming_acl_connect(uint8_t in_acl_conn)
{
    s_is_incoming_acl_connect = in_acl_conn;     
}
#endif
...
void app_bt_enable_complete_wrap( hci_unit_t *unit)
{
    ...
    {
        os_printf("bt write stored link key\r\n");
        cmd[0] = i;
...
#ifdef SUPPORT_BAIDU_APK
void app_bt_read_remote_features_set_auto_conn(void)
{
    app_handle_t sys_hdl = app_get_sys_handler();
    sys_hdl->auto_conn_status = INITIATIVE_CONNECT_START;
    app_bt_reset_policy_iocap();
...
#if 0
//return 0=configer,1=phone,2=tester,3=8852
uint8 app_get_best_offset_level(void)
{
    app_env_handle_t env_h = app_env_get_handle();
...
uint8 app_get_best_offset(void)
{
    app_env_handle_t env_h = app_env_get_handle();
