...
uint8_t  calib_tx_ty2;           //@XER_0x33<7:0>
...
void tx_calibration_deinit(void)
{
    ...
    XVR_ANALOG_REG_BAK[11] &= 0x000001f0;  // xvrB[8:4] is cali value;
    XVR_ANALOG_REG_BAK[11] = XVR_ANALOG_REG_BAK[11] | (xvr_reg_0x0b_bak&0xfffffe0f);
    xvr_reg_set(ADDR_XVR_0x0B, XVR_ANALOG_REG_BAK[11]);
    
    XVR_ANALOG_REG_BAK[12] &= 0x000003ff;  // xvrC[9:0] is cali value;
    XVR_ANALOG_REG_BAK[12] = XVR_ANALOG_REG_BAK[12] | (xvr_reg_0x0c_bak&0xfffffc00);
    xvr_reg_set(ADDR_XVR_0x0C, XVR_ANALOG_REG_BAK[12]);
...
uint32_t tx_iq_phase_imbalance_calibration(void)
{
    ...
    CALIB_DBG("calib_tx_phase_comp = 0x%02X, calib_tx_ty2 = 0x%02X\r\n", calib_tx_phase_comp, tx_ty2);
...
void rx_rssi_calibration_old(void)
{
    ...
    uint32_t xvr_reg_0x25     = *(volatile uint32_t*)(ADDR_XVR_0x25);
    ...
    *(volatile uint32_t*)ADDR_XVR_0x25 = xvr_reg_0x25;
    xvr_reg_set(ADDR_XVR_0x07, xvr_reg_0x07_bak);
    xvr_reg_set(ADDR_XVR_0x08, xvr_reg_0x08_bak);
...
void rx_rssi_calibration(void)
{
    ...
    uint32_t xvr_reg_0x07_bak = xvr_reg_get(ADDR_XVR_0x07);
    uint32_t xvr_reg_0x08_bak = xvr_reg_get(ADDR_XVR_0x08);
    uint32_t xvr_reg_0x24     = *(volatile uint32_t*)(ADDR_XVR_0x24);
    uint32_t xvr_reg_0x25     = *(volatile uint32_t*)(ADDR_XVR_0x25);
    ...
    do
    {
        ...
        do
        {
            ...
            *(volatile uint32_t*)ADDR_XVR_0x25 = 0x2400;
            ...
            adc_rssi = reg_bits_get(ADDR_XVR_0x12, 8,  12);

            //CALIB_DBG("[RSSI-%d][%d]: %d, %d\n", rssi, i, t1[i], t2[i]);
            //CALIB_DBG("[%s]: %d, %0x\r\n", __func__, rx_gain != 0x3FF, rx_gain);
            err += (rx_gain != 0x3FF) && (adc_rssi <= 2);
...
void tx_calib_info(void)
{
    os_printf("[CALIB]: %d, %d, %d, %d, %d, %d, %d, %d, %d\r\n",
              calib_tx_i_doff,
              calib_tx_q_doff,
              calib_tx_i_dc_offset,
              calib_tx_q_dc_offset,
