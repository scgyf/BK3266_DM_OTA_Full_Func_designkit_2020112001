...
void BK3000_GPIO_Initial (void) 
{
    ...
    /// GPIO Debug Message
    BK3000_GPIO_8_CONFIG  = 1<<sft_GPIO_FUNCTION_ENABLE;    ///(IC GPIO4)
    BK3000_GPIO_9_CONFIG  = 1<<sft_GPIO_FUNCTION_ENABLE;    ///(IC GPIO5)
    BK3000_GPIO_12_CONFIG = 1<<sft_GPIO_FUNCTION_ENABLE;   ///(IC GPIO8)
    BK3000_GPIO_13_CONFIG = 1<<sft_GPIO_FUNCTION_ENABLE;   ///(IC GPIO9)
    ...
    BK3000_GPIO_19_CONFIG = 1<<sft_GPIO_FUNCTION_ENABLE; //slot ctrl
    BK3000_GPIO_20_CONFIG = 1<<sft_GPIO_FUNCTION_ENABLE; //sync find(IC GPIO13)
...
*/
    ...
    //BK3000_GPIO_GPIODCON |= 0xf0;

    //Perial Mode 1 function
    BK3000_GPIO_GPIOCON = 0;
}

void DEBUG_MSG(unsigned char val)
{
    #if 0
    BK3000_GPIO_22_CONFIG = 0;	/**< capture pin */
    BK3000_GPIO_DEBUGMSG = val&0XFF;
    BK3000_GPIO_22_CONFIG = 1<<sft_GPIO_OUTPUT;
    #else
    BK3000_GPIO_22_CONFIG = 0 << sft_GPIO_OUTPUT;
    if(val & 0x01)
    {
        BK3000_GPIO_8_CONFIG = ((val>>0)&0X01)<<sft_GPIO_OUTPUT;        ///(IC GPIO4)
    }
    else if(val & 0x02)
    {
        BK3000_GPIO_9_CONFIG = ((val>>1)&0X01)<<sft_GPIO_OUTPUT;        ///(IC GPIO5)
    }
    else if(val & 0x03)
    {
        BK3000_GPIO_12_CONFIG = ((val>>2)&0X01)<<sft_GPIO_OUTPUT;    ///(IC GPIO8)
    }
    else if(val & 0x04)
    {
        BK3000_GPIO_13_CONFIG = ((val>>3)&0X01)<<sft_GPIO_OUTPUT;    ///(IC GPIO9)
    }
    BK3000_GPIO_22_CONFIG = 1 << sft_GPIO_OUTPUT;
    #endif
}
